...
local textify = {
  ...
  textifyBoolean = { pattern = '\\v(true|false)', group = 'Boolean' },
  textifyKeyword = { pattern = '\\v(if|else|while|for|const|let)', group = 'Keyword' },
  textifyMacro = { pattern = '\\v(^#.*|^@.*|function)', group = 'Macro' },
  textifyReturn = { pattern = '\\v(return|^@.*|DONE)', group = 'Return' },
  textifyLocal = { pattern = '\\v(local|for|end)', group = 'luaLocal' },
